...
def parse_arguments():
    ...
    parser.add_argument(
        "--save-vis", action="store_true", help="Save visualization images"
    )
...
def inference(args):
    ...
    pred = 1 / pred
    random_name = f"result_{random.randint(0, 1000000)}.jpg"

    np.save(join(args.output,f"{random_name}.npy"),pred)
    ...
    if args.save_vis:
        ...
        Image.fromarray(artifact).save(join(args.output,random_name))
