...
- `run_eval.sh` 默认保存 RGB/raw/pred 可视化图，默认不删除逐样本 `.npy`。
...
`run_eval.sh` 默认保存可视化图并保留逐样本 `.npy`。如果第三个位置参数为 `true`，脚本会在评估结束后删除逐样本 `.npy`，保留 CSV/JSON 指标和可视化图。若不需要可视化，可设置 `SAVE_VIS=false`。
...
- `run_eval.sh` 默认向 `infer.py` 传入 `--save-vis`，保存 RGB/raw/pred 横向拼接的 `*_vis.jpg`。
